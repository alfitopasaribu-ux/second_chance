...
async function handler(req, res) {
    ...
    try {
        const { conversation_id } = await (0, auth_1.parseBody)(req);
        if (!conversation_id)
            return (0, auth_1.sendJson)(res, 400, { error: 'conversation_id wajib diisi' });
        // Ambil semua pesan user dari percakapan ini
        const messages = await (0, neon_1.query)(`SELECT sender, message FROM messages 
       WHERE conversation_id = $1 ORDER BY created_at ASC`, [conversation_id]);
        if (!messages.length) {
            return (0, auth_1.sendJson)(res, 400, { error: 'Tidak ada pesan untuk dianalisis' });
        }
        const userMessages = messages.filter((m) => m.sender === 'user').map((m) => m.message);
        const conversationText = messages.map((m) => `${m.sender === 'user' ? 'Pengguna' : 'AI'}: ${m.message}`).join('\n');
...
- overall_score: Skor keseluruhan kemampuan komunikasi emosional`;
        ...
        const existing = await (0, neon_1.query)('SELECT id FROM ai_feedback WHERE conversation_id = $1 LIMIT 1', [conversation_id]);
        let feedback;
        if (existing.length > 0) {
            const result = await (0, neon_1.query)(`UPDATE ai_feedback SET 
         ...
         WHERE conversation_id = $9 RETURNING *`, [
            ...
            const result = await (0, neon_1.query)(`INSERT INTO ai_feedback 
        ...
        await (0, neon_1.query)('UPDATE conversations SET is_completed = true WHERE id = $1', [conversation_id]);
        return (0, auth_1.sendJson)(res, 200, { success: true, data: feedback });
    }
    catch (error) {
        console.error('Analysis error:', error);
        return (0, auth_1.sendJson)(res, 500, { error: 'Gagal menganalisis percakapan', detail: error.message });
    }
}
