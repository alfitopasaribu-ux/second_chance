...
async function handler(req, res) {
    ...
    try {
        const { conversation_id, message, category, scenario_context, message_history } = await (0, auth_1.parseBody)(req);
        if (!conversation_id || !message) {
            return (0, auth_1.sendJson)(res, 400, { error: 'conversation_id dan message wajib diisi' });
        }
        // Simpan pesan user
        await (0, neon_1.query)('INSERT INTO messages (conversation_id, sender, message, emotional_tone) VALUES ($1, $2, $3, $4)', [conversation_id, 'user', message, 'neutral']);
        // Update total messages
        await (0, neon_1.query)('UPDATE conversations SET total_messages = total_messages + 1 WHERE id = $1', [conversation_id]);
        // Build system prompt
...
- Boleh menggunakan kata-kata informal seperti "sih", "lho", "ya", "kok"`;
        ...
        // Simpan respons AI
        await (0, neon_1.query)('INSERT INTO messages (conversation_id, sender, message, emotional_tone) VALUES ($1, $2, $3, $4)', [conversation_id, 'ai', aiResponse, emotionalTone]);
        await (0, neon_1.query)('UPDATE conversations SET total_messages = total_messages + 1 WHERE id = $1', [conversation_id]);
        return (0, auth_1.sendJson)(res, 200, {
            success: true,
            data: {
        ...
        return (0, auth_1.sendJson)(res, 500, { error: 'Gagal mendapatkan respons AI', detail: error.message });
    }
}
function analyzeEmotionalTone(text) {
    const lower = text.toLowerCase();
    if (lower.includes('marah') || lower.includes('!') && lower.includes('tidak'))
        return 'angry';
    if (lower.includes('sedih') || lower.includes('menangis') || lower.includes('hancur'))
        return 'sad';
    if (lower.includes('senang') || lower.includes('bahagia') || lower.includes('syukur'))
        return 'happy';
    if (lower.includes('bingung') || lower.includes('...') || lower.includes('entah'))
        return 'confused';
    if (lower.includes('kecewa') || lower.includes('harap') || lower.includes('menyesal'))
        return 'disappointed';
    if (lower.includes('takut') || lower.includes('khawatir') || lower.includes('cemas'))
        return 'anxious';
    return 'neutral';
}
